...
/***********************************************************************
	Couples the lattice points to the finite elements by attributing 
	the FE index to each lattice point (i,j,k).
	Valid only for tetrahedral elements.
***********************************************************************/
        ...
        for (k=element_min[elem][2];k<=element_max[elem][2];k++)
        {
            for (j=element_min[elem][1];j<=element_max[elem][1];j++)
            {
                for (i=element_min[elem][0];i<=element_max[elem][0];i++)
                {
                    ...
                    xcoord=i*Cell_diam+Global_min[0];
                    ycoord=j*Cell_diam+Global_min[1];
                    zcoord=k*Cell_diam+Global_min[2];
