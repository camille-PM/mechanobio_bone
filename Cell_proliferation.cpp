...
void Cell_proliferation(char cells_prol[LATTICE_X][LATTICE_Y][LATTICE_Z], short age_prol[LATTICE_X][LATTICE_Y][LATTICE_Z],int element_local_min [NUMBER_ELEMS][3],
	int element_local_max[NUMBER_ELEMS][3],int lattice_point_element[LATTICE_X][LATTICE_Y][LATTICE_Z], float stimulus_prol[NUMBER_ELEMS], bool graft[LATTICE_X][LATTICE_Y][LATTICE_Z], int iter)
{
    int elem, elem2;
    ...
    int r1,r2,r3;
    int cell; // cell type
    const int critical_time = 15; // activity latency period
				...
				for (k=kmin;k<=kmax;k++) {
					if (lattice_point_element[i][j][k]==elem+1) {
						switch (cells_prol[i][j][k]) {
							case 1:
								if(iter < critical_time || graft[i][j][k]) { // check if in graft presence
									MSCs=MSCs+1;
								}
								break;
							case 2:
...
//			cout << "MSCs to proliferate: " << MSCs_to_prol << endl;
			...
			******************************************/
			...
			while (proliferated<MSCs_to_prol) {
				...
				if (lattice_point_element[i][j][k]==elem+1 && (iter < critical_time || graft[i][j][k])) // if still in same element and in graft presence (or before end of latency period)
				{
